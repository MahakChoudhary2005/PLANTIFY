...
public class LogInServiceImpl implements LogInService {
    ...
    public String logInAccount(LogInData loginData) throws LogInException {
        ...
        if (currentUserOptional.isPresent()) {
            // Remove old session automatically
            currentUserSessionDAO.delete(currentUserOptional.get());
        }

        // Check password
        if ((newSignUp.getUserId() == loginData.getUserId())
                && (newSignUp.getPassword().equals(loginData.getPassword()))) {

            String key = RandomString.getRandomNumberString();

            CurrentUserSession currentUserSession = new CurrentUserSession(newSignUp.getUserId(), key,
                    LocalDateTime.now());
